...
Remote host profiles accept literal unicast IPv4 addresses only and are intended for controlled LAN hosts. The VM list always retains the local computer and can include multiple remote hosts at the same time. Each launch starts with the local host only and does not automatically connect saved profiles. Selecting a profile only displays it. Every explicit connection action reruns the complete IPv4, identity, WMI/DCOM, and TCP 2179 diagnostic before confirmation; the remote host is added only when that fresh result confirms management access.
...
WMI/DCOM and TCP 2179 are diagnosed separately. If management succeeds while TCP 2179 fails, VM management remains available and the console action is disabled with its reason. If one remote management connection is lost, ExHyperV retains that host's last snapshot as stale read-only data, blocks writes to that host, and starts capped exponential-backoff reconnection. The local host and other connected remote hosts remain usable. An explicit disconnect removes only the selected remote host; the local host is never removed.

Remote VM details reuse the local layout for supported data and lifecycle actions. Features that permanently depend on the ExHyperV computer's hardware or file system, including create, rename, delete, and open-folder actions, are hidden for remote VMs; temporarily unavailable supported actions remain visible, disabled, and explain the reason in a tooltip.
...
The console window implements RDP connections based on MsRdpEx. It connects to `localhost:2179` for the local host or to the owning remote host's IPv4 address on TCP 2179. Console availability is independent from that host's WMI/DCOM management availability.
...
When a new console is opened, ExHyperV asks whether to use one monitor or all local monitors. **Use all monitors** requires Enhanced Session mode, negotiates the local monitor topology with the system RDP control, and expands the console across the local virtual desktop so each remote monitor maps to its corresponding physical monitor. If Enhanced Session is unavailable, ExHyperV can fall back to a single-monitor basic session after confirmation. The choice applies only to that console window and is not saved as a global preference. If the same VM already has an open console, ExHyperV activates the existing window without asking again. After Windows locks and unlocks the session, or the display topology changes, ExHyperV waits for the monitor layout to stabilize, renegotiates the RDP topology when needed, and reapplies the multi-monitor full-screen bounds while the option remains active. ExHyperV keeps the native mstscax connection bar visible and pinned in multi-monitor full-screen mode. The console toolbar's redetect button can force the same topology negotiation after a monitor is unplugged or reconnected.
