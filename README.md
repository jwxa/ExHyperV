...
Remote host profiles accept literal unicast IPv4 addresses only and are intended for controlled LAN hosts. The VM list always retains the local computer and can include multiple remote hosts at the same time. Each launch starts with the local host only and does not automatically connect saved profiles. Selecting a profile only displays it; a remote host is added to the VM list only after a successful diagnostic and explicit connection action.
