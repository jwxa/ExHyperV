...
- 本地计算机始终存在于虚拟机列表，并可同时连接多台远程宿主；每台远程宿主都来自已保存的配置。
- 应用每次启动只建立本机会话，不自动连接任何已保存的远程宿主。
- 选择配置不会建立连接；每次点击“连接到此主机”都会重新执行完整的 IPv4、身份、WMI/DCOM 和 TCP 2179 检测，再请求确认。只有最新结果确认管理通道可用时，目标远程宿主才会原子加入会话注册表。

## 2. 通道与部分可用

| 通道 | 用途 | 检测方式 | 失败后的行为 |
|---|---|---|---|
| WMI/DCOM | Hyper-V 查询、虚拟机生命周期、配置向导 | 实际查询 `root\virtualization\v2` | 不能连接目标宿主，或该宿主进入旧数据/重连状态 |
...
3. 保存配置。保存和选中配置都不会建立远程连接。
4. 执行检测，分别查看 IPv4、身份、WMI/DCOM 和 TCP 2179 结果及中文日志。
5. WMI/DCOM 成功后执行连接。ExHyperV 会建立候选会话并读取基础快照。
6. 候选连接和快照都成功后，目标宿主才会一次性加入虚拟机列表；失败时本机和其他已连接宿主保持不变。
7. 虚拟机操作按所属宿主路由；操作前核对宿主分组名称和 IPv4 地址。

## 5. 远程功能支持矩阵

| 功能 | 本地宿主 | 远程宿主 | 说明 |
|---|---:|---:|---|
| 虚拟机列表和状态 | 支持 | 支持 | 每个宿主通过自己的 WMI 上下文独立读取 |
| 启动、正常关闭、强制关闭、重启 | 支持 | 支持 | 写入绑定所属宿主和会话代次，重连或断开后旧结果不会生效 |
| 基本/增强控制台 | 支持 | 条件支持 | 需要所属宿主的 TCP 2179 通道 |
| 高级虚拟机设置 | 支持 | 不支持 | 远程详情不展示永久未迁移的设置入口 |
| 宿主硬件页面 | 支持 | 不支持 | 依赖本机硬件、注册表或系统接口 |
| 虚拟交换机 | 支持 | 不支持 | 尚未迁移到远程宿主上下文 |
| PCIe 直通 | 支持 | 不支持 | 依赖本机设备接口 |
| USB 直通 | 支持 | 不支持 | 依赖本机设备和后台服务 |
| 本地文件或磁盘选择 | 支持 | 不支持 | 路径属于运行 ExHyperV 的计算机；远程 VM 隐藏创建、重命名、删除和打开文件夹入口 |

能力状态分为“暂时不可用”和“永久不支持”。WMI/DCOM 掉线、正在重连或 TCP 2179 不可用等暂时状态会保留对应操作，置灰并通过 Tooltip 说明原因；永久依赖本机硬件、本机文件系统或尚未提供远程实现的入口不会出现在远程详情中。

## 6. 断线与自动重连

某台远程宿主的 WMI 操作出现网络、RPC 或超时类错误后：

1. 保留最后一份宿主快照，并明确标记为旧数据。
2. 禁止虚拟机写入和新控制台连接。
3. 保留该远程宿主，其余远程宿主和始终存在的本机继续独立工作。
...
受控断线验收时，应从外部持续阻断目标宿主网络，直到运行器明确报告已观察到至少两级递增退避后，再恢复网络。验收报告必须同时证明：旧数据状态、旧数据期间写入被拒绝、目标宿主仍在注册表、本机与其他宿主不受影响、退避递增且不超过 30 秒、恢复后产生新的会话代次、基础快照刷新，以及能力矩阵恢复。
...
- 网络恢复后可以等待自动重连或选择立即重试；主动断开只移除所选远程宿主，本机始终保留。
...
- 只有虚拟机列表、四项生命周期操作和控制台接入远程宿主上下文；其他页面保持本地专属。
...
仓库提供独立的 `tests/ExHyperV.IntegrationTests` 验收运行器。它复用产品的 Windows IPv4、WMI/DCOM、TCP 2179、多宿主注册表、VM 查询和控制台捕获适配器；默认不会访问网络：
...
默认只读验收包含配置保存/重载、两通道诊断、只读预检、原子连接、基础快照、真实远程 VM 列表、TCP 2179 控制台捕获和主动断开第一远程宿主。设置 `EXHYPERV_INTEGRATION_SECOND_HOST=<IPv4>` 后，运行器还会诊断并连接第二宿主，证明本机、第一宿主和第二宿主同时存在，通过第二宿主上下文读取真实 VM 列表，再只断开第二宿主并验证本机和第一宿主仍然保留。可用 `EXHYPERV_INTEGRATION_SECOND_DISPLAY_NAME` 指定显示名；当前身份模式可省略 `EXHYPERV_INTEGRATION_SECOND_PROFILE_ID`，Credential Manager 模式必须提供不同的固定 GUID，并预先将第二宿主凭据保存到 `ExHyperV/RemoteHost/<第二宿主GUID>`。

未提供第二宿主时，“两台远程宿主并行”阶段明确记为“跳过”，报告只能是部分通过，不能作为多宿主实机验收证据。以下危险开关相互独立，只有值精确为中文 `确认` 才会启用：
...
TCP 2179 可用和不可用是两个独立的受控验收场景，必须分别留存报告。可用场景证明控制台捕获绑定所属宿主的 IPv4、`HostId` 和会话代次；不可用场景必须在 WMI/DCOM 仍可用时临时阻断 TCP 2179，并证明真实 VM 读取及 VM 读写能力保持可用、控制台能力置灰、控制台捕获被拒绝，且原因明确包含 `TCP 2179`。
...
实机验收必须记录目标地址、认证模式、各阶段状态、WMI/DCOM 和 TCP 2179 错误码、各目标宿主会话代次、VM 数量、控制台目标、主动断开结果、配置步骤/回滚路径以及断线恢复代次。若目标在凭据层之前不可达，应保留失败报告，不得把探测结果伪装成成功。
