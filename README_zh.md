...
远程配置仅接受单播 IPv4 字面地址，用于受控局域网宿主。ExHyperV 的虚拟机列表始终保留本地计算机，并可同时加入多台远程宿主。每次启动只加载本机，不会自动连接已保存的远程配置；选中配置只显示详情。每次点击“连接到此主机”都会重新执行完整的 IPv4、身份、WMI/DCOM 和 TCP 2179 检测，再弹出连接确认；只有最新结果确认管理通道可用时，目标远程宿主才会加入虚拟机列表。

默认身份模式直接使用当前 Windows 身份，不读取密码。也可以临时输入显式凭据；只有选中 **记住凭据** 时，密码才会保存到 Windows 凭据管理器。配置文件仅包含显示名称、IPv4 地址、身份模式、可选用户名和凭据引用，不包含密码。

WMI/DCOM 与 TCP 2179 分别检测。当管理通道成功而 TCP 2179 失败时，虚拟机管理仍可使用，控制台操作会置灰并显示原因。某台远程宿主的管理连接中断后，ExHyperV 会保留该宿主断线前快照并标记为只读旧数据、禁止对其写入，同时按有上限的指数退避自动重连；本机和其他已连接宿主不受影响。用户主动断开时只移除目标远程宿主，本机始终保留。

远程虚拟机详情对已支持的数据和生命周期操作复用本机布局。创建、重命名、删除、打开文件夹等永久依赖 ExHyperV 所在计算机硬件或文件系统的入口在远程虚拟机上直接隐藏；暂时不可用的已支持操作仍保留并置灰，通过 Tooltip 说明原因。
...
控制台窗口基于 MsRdpEx 实现 RDP 连接。本地宿主连接 `localhost:2179`，远程虚拟机连接其所属宿主 IPv4 地址的 TCP 2179；控制台是否可用与该宿主的 WMI/DCOM 管理通道是否可用相互独立。
